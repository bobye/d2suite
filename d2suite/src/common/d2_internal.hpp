...
namespace d2 {
  ...
  namespace internal {
    ...
    void get_dense_if_need(const Block<ElemType> &block, real_t **X, const size_t num_of_copies = 1);
    
    template <size_t D>
    void get_dense_if_need(const Block<Elem<def::WordVec, D> >&block, real_t **X,
			   const size_t num_of_copies = 1) {
      *X=new real_t[D * block.get_col() * num_of_copies];
      for (size_t i=0; i<block.get_col(); ++i) {
	real_t* p=(*X) + i*D*num_of_copies;
	real_t* q=&block.meta.embedding[D*block.get_support_ptr()[i]];
	for (size_t d=0; d<D*num_of_copies; ++d)
	  p[d] = q[d / num_of_copies];
      }	
    }
	...
	*X=new real_t[D * block.get_col() * num_of_copies];
	for (size_t i=0; i<block.get_col(); ++i) {
	  real_t* p=(*X) + i*D*num_of_copies;
	  real_t* q=&block.get_support_ptr()[D*i];
	  for (size_t d=0; d<D*num_of_copies; ++d)
	    p[d] = q[d / num_of_copies];
	}	
      }
    ...
    template <typename ElemType>
    void release_dense_if_need(const Block<ElemType> &block, real_t **X, const size_t num_of_copies = 1);
